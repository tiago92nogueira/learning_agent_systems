...
class MainFix:
    ...
    def __init__(self, master: tk.Tk, params_file: str = "parametros.json"):
        self.master = master
        master.title("Simulação Automática do Agente no Labirinto")
        ...
        self.CELL_SIZE = 25
        self.SPEED = 800  # Velocidade do loop automático em ms
        ...
        if self._load_simulation():
            ...
            self.master.after(500, self._auto_run_step)  # inicia loop automático
    ...
    def _load_simulation(self):
        ...
        try:
            ...
            if not self.motor:
                messagebox.showerror("Erro", "Não foi possível criar o motor de simulação.")
                self.master.destroy()
                return False
    ...
    def _auto_run_step(self):
        ...
        if terminou:
            status = "Objetivo Alcançado!" if self.agente.age() == "FIM" else "Limite de Passos Atingido ou Bloqueio."
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    app = MainFix(root, "parametros.json")
    root.mainloop()