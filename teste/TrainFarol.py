...
def calcular_fitness(agente, sensor, colisoes, max_passos):
    dist = sensor.calcular_prioridade(agente._pos_atual, 0)
    max_dist = agente.lab_rows + agente.lab_cols
    score_dist = (max_dist - dist) / max_dist

    exploracao = len(agente.posicoes_visitadas)

    penal_colisoes = colisoes * 50
    penal_loops = agente.movimentos_repetidos * 15 if hasattr(agente, "movimentos_repetidos") else 0
    ...
    if agente.chegou_ao_fim:
        return 300000 + (max_passos - agente.passos_dados) * 800

    return max(fitness, 1)


def encontrar_start_farol(map_data):
    start = (0, 0)
    farol = (0, 0)
    lab_rows, lab_cols = len(map_data), len(map_data[0])
    ...
    return start, farol, (lab_rows, lab_cols)


def treinar(num_geracoes, tam_populacao):
    try:
        with open("parametersFarol.json", 'r') as f:
            params = json.load(f)
    except FileNotFoundError:
        print("Erro: parametersFarol.json não encontrado.")
        return

    labirinto = params['labirinto']
    start_pos, end_pos, lab_size = encontrar_start_farol(labirinto)
    ...
    if os.path.exists("melhor_cerebroFarol.json"):
        try:
            melhor_rede_anterior = NeuralNet.load_from_file("melhor_cerebroFarol.json")
    ...
    for geracao in range(num_geracoes):
        ...
        for cerebro in populacao:
            ...
            ambiente = AmbienteFarol(labirinto, agente, Sensor)
    ...
    if melhor_rede_global:
        melhor_rede_global.save_to_file("melhor_cerebroFarol.json")
        print("\nTreino Concluído. Melhor modelo guardado em 'melhor_cerebroFarol.json'")
...
if __name__ == "__main__":
    ...
    treinar(num_geracoes=200, tam_populacao=150)